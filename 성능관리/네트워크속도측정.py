...
def shell():
    ...
    printer('근거리 ISP 정보를 검색 중입니다...', quiet)
    try:
        speedtest = Speedtest(
        ...
        printer('근거리 ISP 정보를 검색하지 못했습니다.', error=True)
    ...
    if args.list:
        try:
            ...
            printer('ISP로부터 테스트를 위한 서버 정보를 받아오지 못했습니다.', error=True)
            raise SpeedtestCLIError(get_exception())
    ...
    printer('%(isp)s (%(ip)s)을 기준으로 테스트를 시작합니다...' % speedtest.config['client'],
            quiet)
 
    if not args.mini:
        printer('ISP로부터 서버 정보를 수신 중입니다...', quiet)
        try:
        ...
        if args.server and len(args.server) == 1:
            ...
            printer('수신된 서버 중 최적의 서버로 ICMP 테스트를 수행합니다...', quiet)
    ...
    printer('호스트:%(sponsor)s 지역:%(name)s 거리:약%(d)0.2fkm RTT:'
            '%(latency)sms' % results.server, quiet)
 
    if args.download:
        printer('다운로드 속도 측정 중', quiet,
                end=('', '\n')[bool(debug)])
        speedtest.download(
            callback=callback,
            threads=(None, 1)[args.single]
        )
        printer('다운로드속도: %0.2f M%s/s' %
                ((results.download / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
    else:
        printer('다운로드 속도 측정을 생략합니다.', quiet)
 
    if args.upload:
        printer('업로드 속도 측정 중', quiet,
                end=('', '\n')[bool(debug)])
        speedtest.upload(
            callback=callback,
        ...
        printer('업로드속도: %0.2f M%s/s' %
                ((results.upload / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
    else:
        printer('업로드 속도 측정을 생략합니다.', quiet)
